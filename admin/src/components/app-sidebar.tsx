            ...
            <SidebarMenu>
              {adminLinks.map((item) => (
                <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton render={<Link href={item.url} />} isActive={pathname === item.url} className="hover:text-primary transition-colors tooltip-content font-sans">
                    <item.icon className="w-4 h-4" />
                    <span className="text-xs uppercase tracking-[0.15em] font-medium">{item.title}</span>
                  </SidebarMenuButton>
                </SidebarMenuItem>
              ))}
            </SidebarMenu>
