    ...
    <PageLayout>
      ...
      <section className="relative w-full h-85 md:h-120 overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
          <p className="font-primary text-[10px] tracking-[0.35em] uppercase text-brand-gold mb-3">Since 1925</p>
          <h1 className="font-secondary text-4xl md:text-6xl text-white leading-tight mb-4">A Legacy of Purity</h1>
          <p className="font-primary text-white/80 text-sm md:text-base max-w-xl leading-relaxed">
      ...
      <section className="max-w-480 mx-auto px-4 md:px-16 lg:px-24 py-16 md:py-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div>
            ...
            <div className="font-primary text-gray-500 text-sm md:text-base leading-relaxed space-y-4">
          ...
          <div className="relative h-105 md:h-130">
            ...
            <div className="absolute -bottom-6 -left-6 bg-brand-brown text-white p-6 w-40 text-center hidden md:block">
              ...
              <p className="font-primary text-[10px] tracking-widest uppercase text-white/80 mt-1">Years of Trust</p>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {values.map(({ title, desc }) => (
              <div key={title} className="bg-white p-6 border border-gray-100">
                ...
                <p className="font-primary text-sm text-gray-500 leading-relaxed">{desc}</p>
              </div>
            ))}
          </div>
        </div>
            ...
            {milestones.map((m, i) => (
              <div key={m.year} className={`flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-8 ${i % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                <div className={`flex-1 ${i % 2 === 0 ? 'md:text-right' : 'md:text-left'}`}>
                  ...
                  <p className="font-primary text-sm text-gray-500 leading-relaxed max-w-sm mx-auto md:mx-0 md:ml-auto">{m.text}</p>
                </div>
      ...
      <section className="bg-brand-brown py-14 text-center">
        <p className="font-primary text-[10px] tracking-[0.3em] uppercase text-brand-gold mb-3">Visit Us</p>
