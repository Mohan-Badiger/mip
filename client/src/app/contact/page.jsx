  ...
  return (
    <PageLayout>
      ...
      <div className="bg-bg-cream border-b border-gray-100 py-10 md:py-16 text-center">
        <p className="font-primary text-[10px] tracking-[0.3em] uppercase text-brand-gold mb-2">{"We're Here For You"}</p>
        <h1 className="font-secondary text-4xl md:text-5xl text-brand-brown tracking-wide">Contact Us</h1>
        <p className="font-primary text-gray-500 text-sm mt-3">Reach out for anything — our team responds within 24 hours.</p>
      </div>
      ...
      <div className="max-w-[1920px] mx-auto px-4 md:px-16 py-12 md:py-16 pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-16">
          ...
          <div className="lg:col-span-1 space-y-8">
            <div>
              <p className="font-primary text-[10px] tracking-[0.3em] uppercase text-brand-gold mb-4">Get In Touch</p>
            ...
            <div className="space-y-5">
              {[
                ...
                <div key={label} className="flex gap-3.5">
                  ...
                  <div>
                    <p className="font-primary text-[9px] tracking-[0.2em] uppercase text-gray-400 mb-0.5">{label}</p>
                    {href ? (
                      <a href={href} className="font-primary text-sm text-brand-brown hover:text-brand-gold transition-colors">{value}</a>
                    ) : (
          ...
          <div className="lg:col-span-2">
            {sent ? (
              <div className="flex flex-col items-center justify-center h-full min-h-[360px] text-center gap-4">
                ...
                <button onClick={() => setSent(false)} className="font-primary text-xs text-brand-gold underline underline-offset-2 tracking-wide mt-2 hover:text-brand-brown transition-colors">Send another message</button>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="font-primary text-[9px] tracking-[0.2em] uppercase text-gray-500 block mb-1.5">Full Name *</label>
                    <input
                      required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                      placeholder="Your name"
                      className="w-full px-4 py-3 border border-gray-200 focus:border-brand-gold focus:outline-none font-primary text-sm text-brand-brown placeholder:text-gray-300 bg-white transition-colors"
                    />
                  </div>
                  <div>
                    <label className="font-primary text-[9px] tracking-[0.2em] uppercase text-gray-500 block mb-1.5">Email Address *</label>
                    <input
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="font-primary text-[9px] tracking-[0.2em] uppercase text-gray-500 block mb-1.5">Phone Number</label>
                    <input
                  ...
                  <div>
                    <label className="font-primary text-[9px] tracking-[0.2em] uppercase text-gray-500 block mb-1.5">Subject</label>
                    <select
                      value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })}
                      className="w-full px-4 py-3 border border-gray-200 focus:border-brand-gold focus:outline-none font-primary text-sm text-brand-brown bg-white transition-colors"
                    >
                ...
                <div>
                  <label className="font-primary text-[9px] tracking-[0.2em] uppercase text-gray-500 block mb-1.5">Message *</label>
                  <textarea
                    required rows={5} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })}
                    placeholder="How can we help you?"
                    className="w-full px-4 py-3 border border-gray-200 focus:border-brand-gold focus:outline-none font-primary text-sm text-brand-brown placeholder:text-gray-300 bg-white transition-colors resize-none"
                  />
                </div>
                <button
                  type="submit"
                  className="flex items-center gap-2 bg-brand-brown hover:bg-brand-gold transition-colors text-white font-primary text-xs font-semibold tracking-[0.2em] uppercase px-10 py-4"
                >
