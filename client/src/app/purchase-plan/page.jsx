        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          {plans.map((plan) => (
            <div key={plan.name} className={`${plan.color} border border-brand-gold/20 p-8 md:p-10 flex flex-col`}>
              <span className="font-primary text-[9px] tracking-[0.25em] uppercase text-brand-gold mb-3">{plan.tag}</span>
              ...
              <ul className="space-y-2.5 mb-8 flex-1">
                {plan.benefits.map((b) => (
                  <li key={b} className="flex items-start gap-2.5">
                    ...
                    <span className={`font-primary text-sm ${plan.name === 'Kanaka Plus' ? 'text-white/85' : 'text-gray-600'}`}>{b}</span>
                  </li>
                ))}
              </ul>
              <Link
                href="/stores"
                className={`inline-flex items-center gap-2 font-primary text-xs font-semibold tracking-[0.2em] uppercase pb-1 border-b transition-colors ${plan.name === 'Kanaka Plus' ? 'text-white border-white/40 hover:border-brand-gold hover:text-brand-gold' : 'text-brand-brown border-brand-gold hover:text-brand-gold'}`}
              >
      ...
      <section className="relative overflow-hidden">
        <div className="bg-brand-brown py-14 md:py-16 px-4 md:px-16 flex flex-col md:flex-row items-center justify-between gap-8 max-w-[1920px] mx-auto">
          <div className="text-center md:text-left">
            ...
            <p className="font-primary text-white/70 text-sm max-w-md leading-relaxed">Visit any MIP showroom or call us to enrol. Our team will help you choose the right plan.</p>
          </div>
